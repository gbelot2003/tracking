var agencias = function($scope, agenciasFactory, $uibModalInstance, $http){
    ...
    $scope.ok = function () {
        agenciasFactory.save($scope.agencia).$promise
            .then(function success(response){
                console.log("desde paquetesAgencia = " + response.id);
                var agencia = {
                    id: response.id,
                    name: response.name
                };
                $uibModalInstance.close(agencia);
            }, function(error){
                $scope.message = false;
...
module.exports = function(app){
    app.controller('paquetesAgenciasController', function($scope, agenciasFactory, $uibModalInstance, $http){
