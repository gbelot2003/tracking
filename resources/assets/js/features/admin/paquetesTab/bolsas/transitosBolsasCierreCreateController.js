var transito = function($scope, $uibModalInstance, id, codeId, transitosBolsasFactory, Upload, $timeout, ngToast, $http, estadosService){
    ...
    $scope.showItemsChange = false;

    $scope.$watch('transitos.estado_id', function(oldD, newD){
        if(oldD != newD){
            console.log($scope.transitos.estado_id);
            if($scope.transitos.estado_id === 8){
                $scope.showItemsChange = true;
            } else {
                $scope.showItemsChange = false;
            }
        }
